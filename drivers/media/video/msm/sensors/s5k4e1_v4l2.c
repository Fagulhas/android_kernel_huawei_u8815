...
static struct msm_camera_i2c_reg_conf s5k4e1_prev_settings[] = {
	...
	{0x30C0, 0x90},/* video_offset[7:4] 3260%12 */
...
/* currently, these 2 empty arrays will not be used, just reserved */
...
uint16_t s5k4e1_read_layer_data(uint16_t layer, uint16_t addr, uint16_t MSBorLSB )
{
	uint16_t otpData = 0;
   ...
   msm_camera_i2c_read(s5k4e1_s_ctrl.sensor_i2c_client, addr, &otpData, MSM_CAMERA_I2C_BYTE_DATA);
...
static uint16_t s5k4e1_read_rg(uint16_t otpAddr, uint16_t MSBorLSB)
{
	uint16_t otpData = 0;
	uint16_t rg = 0;
		
    /*read RG */
	otpData = s5k4e1_read_layer_data(28, otpAddr, MSBorLSB);
	rg = otpData << 12;

	otpData = s5k4e1_read_layer_data(29, otpAddr, MSBorLSB);
	rg |= otpData << 8;

	otpData = s5k4e1_read_layer_data(30, otpAddr, MSBorLSB);
	rg |= otpData << 4;

	otpData = s5k4e1_read_layer_data(31, otpAddr, MSBorLSB);
	rg |= otpData ;

	return rg;
}

static uint16_t s5k4e1_read_bg(uint16_t otpAddr, uint16_t MSBorLSB)
{
	uint8_t otpData = 0;
	uint16_t bg = 0;
	
    /*read RG */
	otpData = s5k4e1_read_layer_data(32, otpAddr, MSBorLSB);
	bg = otpData << 12;

	otpData = s5k4e1_read_layer_data(33, otpAddr, MSBorLSB);
	bg |= otpData << 8;

	otpData = s5k4e1_read_layer_data(34, otpAddr, MSBorLSB);
	bg |= otpData << 4;

	otpData = s5k4e1_read_layer_data(35, otpAddr, MSBorLSB);
	bg |= otpData ;
...
*/
static int s5k4e1_otp_check_data(uint16_t *pAddr, uint16_t *pMSBorLSB)
{
	...
	msm_camera_i2c_write(s5k4e1_s_ctrl.sensor_i2c_client, 0x310C, 18, MSM_CAMERA_I2C_BYTE_DATA);

	/* read 0x310D LSB */
	 otpAddr = 0x310D;
	 msm_camera_i2c_read(s5k4e1_s_ctrl.sensor_i2c_client, otpAddr, &otpData, MSM_CAMERA_I2C_BYTE_DATA);
...
static int s5k4e1_otp_data_read(void)
{
	...
	/*check awb */
	if(0== s5k4e1_otp_check_data(&otpAddr, &MSBorLSB))
	{
		return -1;
	}
	...
	rg = s5k4e1_read_rg(otpAddr, MSBorLSB);

	bg = s5k4e1_read_bg(otpAddr, MSBorLSB);
    ...
    g_modeID = s5k4e1_read_layer_data(26, otpAddr, MSBorLSB);
	
	printk("%s rg = 0x%04x, bg = 0x%04x ,modeID = %d, DataTime:%d%d%d%d%d%d \n",
		__func__, rg, bg, g_modeID,
		s5k4e1_read_layer_data(18, otpAddr, MSBorLSB), s5k4e1_read_layer_data(19, otpAddr, MSBorLSB),
		s5k4e1_read_layer_data(20, otpAddr, MSBorLSB), s5k4e1_read_layer_data(21, otpAddr, MSBorLSB),
		s5k4e1_read_layer_data(22, otpAddr, MSBorLSB), s5k4e1_read_layer_data(23, otpAddr, MSBorLSB));
